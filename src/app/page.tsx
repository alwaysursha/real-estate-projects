...
export default function Home() {
  const [apartmentFavorites, setApartmentFavorites] = useState<(string | number)[]>([]);
  const [villaFavorites, setVillaFavorites] = useState<(string | number)[]>([]);
  
  // Get properties by category
  const apartmentProperties = getPropertiesByCategory('apartment');
  const villaProperties = getPropertiesByCategory('villa');

  const toggleApartmentFavorite = (id: string | number, e: React.MouseEvent) => {
    e.preventDefault();
    e.stopPropagation();
    if (apartmentFavorites.includes(id)) {
      setApartmentFavorites(apartmentFavorites.filter(favId => favId !== id));
    } else {
      setApartmentFavorites([...apartmentFavorites, id]);
    }
  };

  const toggleVillaFavorite = (id: string | number, e: React.MouseEvent) => {
    e.preventDefault();
    e.stopPropagation();
    if (villaFavorites.includes(id)) {
      setVillaFavorites(villaFavorites.filter(favId => favId !== id));
    } else {
      setVillaFavorites([...villaFavorites, id]);
    }
  };
                    ...
                    <div className="bg-white rounded-lg shadow-md overflow-hidden h-full property-card">
                      <div className="relative pb-[66.666667%]">
                        ...
                        <span className={`absolute top-4 right-4 text-white px-3 py-1 rounded-md ${
                          property.status === 'Now Selling' || property.status === 'Ready to Move'
                            ? 'bg-green-600' 
                            : property.status === 'Coming Soon' || property.status === 'Off-Plan'
                              ? 'bg-yellow-500' 
                              : 'bg-red-600'
                        }`}>
                          {property.status}
                      ...
                      <div className="p-6 flex flex-col flex-grow">
                        ...
                        <div className="mt-auto flex items-center justify-between pt-2 border-t border-gray-100 mt-4">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <button 
                              onClick={(e) => toggleApartmentFavorite(property.id, e)}
                              className={`${apartmentFavorites.includes(property.id) ? 'text-red-500' : 'text-gray-500 hover:text-red-500'} transition-colors`}
                              aria-label="Save to favorites"
                            >
                              <svg className="w-4 h-4" fill={apartmentFavorites.includes(property.id) ? "currentColor" : "none"} stroke="currentColor" viewBox="0 0 24 24">
                      ...
                      <div className="p-6 flex flex-col flex-grow">
                        ...
                        <div className="mt-auto flex items-center justify-between pt-2 border-t border-gray-100 mt-4">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <button 
                              onClick={(e) => toggleVillaFavorite(property.id, e)}
                              className={`${villaFavorites.includes(property.id) ? 'text-red-500' : 'text-gray-500 hover:text-red-500'} transition-colors`}
                              aria-label="Save to favorites"
                            >
                              <svg className="w-4 h-4" fill={villaFavorites.includes(property.id) ? "currentColor" : "none"} stroke="currentColor" viewBox="0 0 24 24">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" />
                              </svg>
                            </button>
                          </div>
